...
function SelectField({ label, value, onChange, options, required, placeholder, error, disabled }: {
  label?: string; value: string; onChange: (v: string) => void;
  options: { label: string; value: string }[]; required?: boolean; placeholder?: string; error?: string; disabled?: boolean;
}) {
  return (
    <div className="flex flex-col gap-1.5">
      {label && <label className={cx("text-sm font-medium text-foreground", disabled && "text-muted-foreground")}>{label}{required && <span className="text-primary mr-1">*</span>}</label>}
      <select value={value} onChange={(e) => onChange(e.target.value)} disabled={disabled}
        className={cx("w-full rounded-xl border border-border bg-input-background px-3.5 py-2.5 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring focus:border-primary", disabled && "opacity-50 cursor-not-allowed")}>
