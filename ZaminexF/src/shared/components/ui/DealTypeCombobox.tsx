...
function DealTypeCombobox({
  ...
  const getDisplayLabel = (name: string) => name === "رهن و اجاره" ? "اجاره" : name;
  const selected = useMemo(() => dealTypes.find((d) => String(d.id) === String(value) || d.displayName === value), [dealTypes, value]);
  const filtered = useMemo(() => fuzzyFilter(dealTypes, q, (d) => d.displayName), [dealTypes, q]);
      ...
      <div className="relative">
        ...
        >
          ...
          <span className={cx("flex-1 truncate", value ? "text-foreground font-medium" : "text-muted-foreground")}>
            {(selected?.displayName ? getDisplayLabel(selected.displayName) : "نوع معامله")}
          </span>
                ...
                filtered.map((d, i) => (
                  <button
                    ...
                    className={cx(
                      ...
                      String(value) === String(d.id) && "bg-primary/5 text-primary font-medium"
                    )}
                  >
                    <Tag size={12} className="text-muted-foreground flex-shrink-0" />
                    <span className="flex-1 text-right">{hl(getDisplayLabel(d.displayName))}</span>
                    {String(value) === String(d.id) && <Check size={12} className="text-primary flex-shrink-0" />}
                  </button>
                ))
              )}
            </div>
