import { ChevronDown, Search, Check, ChevronUp, MoreVertical, X, MapPin } from "lucide-react";
...
function DistrictCombobox({
  ...
  const filtered = useMemo(() => fuzzyFilter(districtsList, q, (d) => d), [districtsList, q]);
    ...
    <div className="space-y-1.5">
      {label && (
        <label className="block text-xs font-medium text-foreground">
          {label}{required && <span className="mr-1 text-destructive">*</span>}
        </label>
      )}
      ...
      {open && (
        <div className="absolute z-50 w-full mt-1 bg-card rounded-xl border border-border shadow-lg overflow-hidden">
          ...
          <div className="max-h-52 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-4">محله‌ای یافت نشد</p>
            ) : (
              filtered.map((d, i) => (
                <button 
                  key={d} 
