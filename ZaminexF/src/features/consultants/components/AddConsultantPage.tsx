...
import { Page, Role, Property, Listing, FollowUp, ConsultantItem, BadgeV, AddConsultantFormState, ConsultantRole } from "../../../shared/lib/types";
...
function AddConsultantPage({
  ...
  onSubmit: (payload: Record<string, any>) => Promise<any>;
  isSubmitting: boolean;
  submitError: string | null;
  districtsList: string[];
}) {
  const [form, setForm] = useState<AddConsultantFormState>({
    firstName: "",
    lastName: "",
    username: "",
    email: "",
  ...
  const setField = <K extends keyof AddConsultantFormState>(
    key: K,
    value: AddConsultantFormState[K]
  ) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!firstName || !lastName || !username || !password || !form.branch.trim()) {
      toast({ type: "error", message: "لطفا نام، نام خانوادگی، نام کاربری، رمز عبور و شعبه را وارد کنید." });
      return;
    }
  ...
  const roleOptions = [{ label: "مشاور", value: "AGENT" }];
  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
      <div className="mb-6 flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        <span className="font-medium text-foreground">افزودن مشاور</span>
      </div>

      <Card className="space-y-5 p-6">
        <div>
          <h2 className="text-base font-semibold">ساخت حساب مشاور جدید</h2>
          <p className="mt-1 text-xs text-muted-foreground">
            یک حساب کاربری جدید برای مشاور ایجاد کرده و شعبه، نقش و دسترسی‌ها را تعیین کنید.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <SelectField
            label="نقش"
            value={form.role}
            onChange={(v) => setField("role", v as ConsultantRole)}
            options={roleOptions}
            required
          />
          <DistrictCombobox
